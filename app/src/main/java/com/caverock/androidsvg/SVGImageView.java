...
public class SVGImageView extends ImageView
{
   ...
   {
      new LoadResourceTask(getContext()).execute(resourceId);
   }
   ...
   {
      new LoadAssetTask(getContext()).execute(filename);
   }
   ...
   {
      Context mContext;
      public LoadResourceTask(Context mContext)
      {
   ...
   {
      Context mContext;
      public LoadAssetTask(Context mContext)
      {
         this.mContext =mContext;
      }
      ...
      protected Picture  doInBackground(String... filename)
      {
         try
         {
            SVG  svg = SVG.getFromAsset(mContext.getAssets(), filename[0]);
            return svg.renderToPicture();
         }
         catch (SVGParseException e)
         {
